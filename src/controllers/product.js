    ...
    try {
        ...
        Categories.find({}, (err, cats) => {
            cats.forEach((cat) => {
                ...
                childName.push(cat.childName[0]);
            });
        });
        ...
        return res.render("products", {
            ...
            categoriesChild: childName,
            hasNextPage: ITEM_PER_PAGE * page < numProduct,
            hasPreviousPage: page > 1,
            nextPage: page + 1,
            previousPage: page - 1,
            lastPage: Math.ceil(numProduct / ITEM_PER_PAGE),
    ...
    if (req.session.cart.totalQty) {
        ...
        for (var id in req.session.cart.items) {
            await Products.findOne({ _id: id })
                .then((product) => {
                    product.buyCounts += parseInt(
                        req.session.cart.items[id].qty
                    );
                    product.save();
                })
